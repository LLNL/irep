...
of arbitrary Lua tables or functions into the well known table.
...
in the range [0,1014] and for I<NRET>, the range [-9,2097142].
(But note that the enforced lower bound for I<NRET> is currently
-1, not -9.)

The I<NPRM> parameter is checked by IREP to verify its range,
then passed into the C<lua_cb_data> structure without additional
processing.
The I<NRET> parameter is more important, however.
